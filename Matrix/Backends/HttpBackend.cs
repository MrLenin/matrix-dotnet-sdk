...
namespace Matrix.Backends
{
	public class HttpBackend : IMatrixApiBackend
	{
		...
		private Uri GetPath(Uri apiPath, bool auth)
		{
            ...
            apiPath	= new Uri(apiPath, (string.IsNullOrEmpty(apiPath.Query) ? "&" : "?") + "access_token=" + _accessToken);
		...
		public MatrixRequestError Post(Uri apiPath, bool authenticate, JToken data, Dictionary<string,string> headers, out JToken result)
        {
		...
		public MatrixRequestError Post(Uri apiPath, bool authenticate, byte[] data, Dictionary<string,string> headers, out JToken result)
		{
            if (headers == null) throw new ArgumentNullException(nameof(headers));
